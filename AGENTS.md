...
2. **If Herdr is available, before spawning any new Herdr agent, stop and ask the user which agent to spawn** (e.g. `codex`, `opencode`, another `claude`). Do not choose one yourself and do not spawn without an explicit answer. Exception on this repo: the **reviewer** role is locked to Codex — no need to ask for that role, only for orchestrator/implementer or other spawns.
