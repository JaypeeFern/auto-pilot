...
only). Deploy the root Compose file with the focused tool documentation; no
local install is needed for normal operation.
...
Query the tool deployment privately from the n8n network, for example with
`node -e "fetch('http://browser:5679/status').then(r=>r.json()).then(j=>console.log(JSON.stringify(j)))"`.
